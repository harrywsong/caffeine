...
client.once('ready', async () => {
    console.log('✅ Bot ready! Updating command channels...');
    
    try {
        // Get channels
        const regularChannel = await client.channels.fetch(REGULAR_COMMANDS_CHANNEL);
        const staffChannel = await client.channels.fetch(STAFF_COMMANDS_CHANNEL);
        
        console.log('📋 Clearing existing messages...');
        
        // Clear existing messages (bulk delete up to 100 messages at a time)
        try {
            const regularMessages = await regularChannel.messages.fetch({ limit: 100 });
            if (regularMessages.size > 0) {
                await regularChannel.bulkDelete(regularMessages);
                console.log(`🗑️ Cleared ${regularMessages.size} messages from regular commands channel`);
            }
        } catch (error) {
            console.log('⚠️ Could not bulk delete from regular channel (messages may be old)');
        }
        
        try {
            const staffMessages = await staffChannel.messages.fetch({ limit: 100 });
            if (staffMessages.size > 0) {
                await staffChannel.bulkDelete(staffMessages);
                console.log(`🗑️ Cleared ${staffMessages.size} messages from staff commands channel`);
            }
        } catch (error) {
            console.log('⚠️ Could not bulk delete from staff channel (messages may be old)');
        }
        
        // Post regular user commands
        console.log('📝 Posting regular user commands...');
        await postRegularCommands(regularChannel);
        
        // Post staff commands
        console.log('📝 Posting staff commands...');
        await postStaffCommands(staffChannel);
        
        console.log('✅ Command channels updated successfully!');
        process.exit(0);
